...
The cancellation rate is computed by dividing the number wiht unbanned users on that day.*/
...
select request_at, count(case when status in ('cancelled_by_client','cancelled_by_driver') then 1 else null end) as cancelled_trip_counts,
